...
// MODIFICATION: Configuration variable to disable image uploading.
// Set to 'false' to only show the demo image and hide upload controls.
const ALLOW_IMAGE_UPLOADING = false;

// MODIFICATION: Configuration for the default demo images.
// These images must be placed in your `public` folder.
// NOTE: Update the dimensions to match your actual demo-image.png.
...
// MODIFICATION: New component to show a simple preview of the uploaded image
const ImagePreviewScene = ({ url, dimensions }) => {
  ...
  return (
    <mesh scale={[width, height, 1]}>
      <planeGeometry />
      {/* Using toneMapped=false to ensure the preview image isn't affected by post-processing like tone mapping */}
...
export default function Home() {
  ...
  // MODIFICATION: New state for holding the preview image URL and dimensions.
  const [preview, setPreview] = useState(null);

  // MODIFICATION: useEffect to clean up blob URLs to prevent memory leaks.
  useEffect(() => {
  ...
  // MODIFICATION: Now just processes the file and sets up the preview.
  const processFile = useCallback((fileToProcess) => {
    ...
    setImageConfig(null); // Clear previous final image
  ...
  // MODIFICATION: This now only handles the backend communication.
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // On success, set the final image config and clear the preview
      ...
      // On failure, clear everything to go back to the dropzone or demo
      setPreview(null);
      setImageConfig(ALLOW_IMAGE_UPLOADING ? null : DEMO_IMAGE_CONFIG);
      setFile(null);
    } finally {
      setIsLoading(false);
    }
  ...
  // Determine what to show: nothing (dropzone), a preview, or the final effect.
  ...
  return (
    <main {...dragProps}>
      ...
      {/* MODIFICATION: The canvas is now permanent to prevent context loss. */}
      ...
      {/* MODIFICATION: The dropzone is now an overlay, shown conditionally. */}
